...
export default {
  ...
  setup(){
    ...
    async function callPoiLookup(messages){
      const candidates = [
        import.meta.env.VITE_PROXY_URL || 'http://localhost:3001',
        'http://localhost:3001',
        'http://localhost:3002'
      ...
      for (const base of candidates) {
        try {
          const res = await fetch(`${base}/api/poi-lookup`, {
            method: 'POST', headers: { 'Content-Type':'application/json' },
            body: JSON.stringify({ messages })
          })
          const j = await res.json().catch(() => null)
          if (j && j.found && j.content) return j.content
        } catch (e) {
          // ignore and try next
        }
      }
    ...
    async function send(){
      ...
      sending.value = true
      try{
        // quick local POI lookup for time-related queries to ensure single-place reply
        const timeKeywords = ['영업시간', '운영시간', '영업', '운영', '몇시', '몇 시', '시간']
        const asksForTime = timeKeywords.some(k => text.toLowerCase().includes(k))
        if (asksForTime) {
          const local = await callPoiLookup([{ role: 'user', content: text }])
          if (local) {
            history.value.push({ role:'assistant', content: local, ts: Date.now() })
            persist()
            sending.value = false
            return
          }
        ...
        let reply = await callOpenAI(msgs)
        // if response is the generic 'not an auto-recommendation' fallback, try POI-only lookup
        if (reply && reply.includes('자동 추천 대상')) {
          const local = await callPoiLookup(msgs)
          if (local) reply = local
          else reply = `어떤 장소의 운영시간을 알고 싶으신가요? 장소명을 입력해 주세요. 예: 'KT&G 상상마당 부산'`
        }
